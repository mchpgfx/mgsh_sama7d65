...
void Screen1_OnShow()
{
    ...
    fpsStr.fn->setFont(&fpsStr, (leFont*)&ArialNarrowMed);
    Screen1_FPSLabel->fn->setVisible(Screen1_FPSLabel, stats_enabled == true);  
...
void Screen1_OnUpdate()
{
        ...
        last_sec_count != sec_count)
    {
        //update fps
#ifdef RTOS_ENABLED
        sprintf(fpsStrBuff, "fps %u\ncpu %u%%", fps, 100 - cpu_free); 
#else
        sprintf(fpsStrBuff, "%u fps", fps);
#endif
        fpsStr.fn->setFromCStr(&fpsStr, fpsStrBuff);    
        Screen1_FPSLabel->fn->setString(Screen1_FPSLabel, (leString*)&fpsStr);          
